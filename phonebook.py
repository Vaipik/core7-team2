...
class AddressBook(UserDict):
        ...
        for record in self.data.values():  # type: Record
            ...
            for field in self.__fields:
                ...
                if isinstance(record_field, list):
                    data_in_field = [
                        data for data in record_field if search in data.value]
    ...
    def show_near_birthdays(self, days: int = 30) -> list:
        ...
        for record in self.data.values():  # type: Record
            ...
            if record.birthday:
                birthday = record.birthday.value.replace(
        ...
        for record in self.data.values():
            for field in self.__fields:
                ...
                if record_field is None:
                    ...
                    # type: List[Phone] | List[Email]
                    page_info.append([f"{field}:", *record_field])
            ...
            if on_page == self.__records_per_page:
                ...
                page_info.append(
                    [f"{current_page} page of {total_pages} pages\n"])
                yield page_info
                page_info.clear()
                current_page += 1
                on_page = 0
    ...
    def show_record_data(self, contact: str) -> list:
        ...
        for field in self.__fields:
            ...
            if record_field is None:
                ...
                # type: List[Phone] | List[Email]
                data.append([f"{field}:", *record_field])
